...
def _schema_errors(corpus: dict[str, Any], schema: dict[str, Any]) -> list[str]:
    validator = Draft202012Validator(schema, format_checker=FormatChecker())
    errors: list[str] = []
    for error in sorted(validator.iter_errors(corpus), key=lambda item: list(item.absolute_path)):
        location = "$"
...
def _integrity_errors(corpus: dict[str, Any], *, require_freeze_ready: bool) -> tuple[list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    for case in cases:
        ...
        if arm == "historical" and eligibility == "qualified":
    ...
    for family_id, members in sorted(family_members.items()):
        qualified_members = [
            ...
            if any(
                ...
                and case.get("arm") == "historical"
                and case.get("eligibility") == "qualified"
                for case in cases
            )
        ]
        if len(qualified_members) > 2:
            warnings.append(
                f"correlated family {family_id!r} has {len(qualified_members)} qualified historical cases; "
                "report family-stratified results and do not treat them as independent wins"
            )
    ...
    strict = require_freeze_ready or corpus.get("corpus_status") in {"frozen", "amended"}
    if strict:
...
def _summary(corpus: dict[str, Any]) -> str:
    ...
    qualified = [
        case
        for case in cases
        if case.get("arm") == "historical" and case.get("eligibility") == "qualified"
    ]
    ...
    return "\n".join(
        [
            ...
            "qualified_repositories=" + ", ".join(f"{key}:{value}" for key, value in sorted(repo_counts.items())),
        ]
    )
...
def main() -> int:
    ...
    integrity_errors, warnings = _integrity_errors(corpus, require_freeze_ready=args.require_freeze_ready)
